...
import { Grid, Typography, Avatar, Card, CardContent, Link } from "@mui/material";
import Layout from "../Layout/Layout";
import sir from "../images/sir.png";
import maam from "../images/maam.png";

const teachers = [
  { name: "xWave Academy", avatar: "X" },  // Use "X" for avatar text
...
const classmates = [
  ...
  { name: "Muneem Ali", avatar: "M" }, // Use text for avatar
  ...
  { name: "Khuda Bux", avatar: "k" }, // Use text for avatar
  

];
const ListItem = ({ name, avatar }) => (
  <Card variant="outlined" sx={{ display: "flex", alignItems: "center", padding: 1, boxShadow: 0 }}>
    {/* If avatar is text, use Avatar component; if image, render an img */}
    {avatar && avatar.length === 1 ? (
      <Avatar sx={{ width: 30, height: 30, bgcolor: "primary.main"   }}>{avatar}</Avatar>
    ) : (
...
const People = () => {
  return (
    <>
      ...
      <Grid container spacing={2} justifyContent="center" sx={{ maxWidth: 600, margin: "auto" }}>
        {/* Teachers Section */}
        <Grid item xs={12}>
